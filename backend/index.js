...
app.get('/api/letters', async (req, res) => {
  try {
    const { campaignId, houseId } = req.query;
    let query = knex('letters').select('*');
    if (campaignId) {
      query = query.where({ campaignId });
    }
    if (houseId) {
      query = query.where({ houseId });
    }
...
app.get('/api/streets', async (req, res) => {
  try {
    const streets = await knex('streets').select('*');
    res.json(streets);
  } catch (error) {
...
app.get('/api/houses', async (req, res) => {
  try {
    const houses = await knex('houses').select('*');
    res.json(houses);
  } catch (error) {
...
// --- Study History ---
app.get('/api/study-history', async (req, res) => {
  try {
    const { studyId } = req.query;
    let query = knex('study_history').select('*');
    if (studyId) {
      query = query.where({ studyId });
    }
...
app.post('/api/study-history', async (req, res) => {
  try {
    const newEntry = req.body;
    const [id] = await knex('study_history').insert(newEntry);
    const entry = await knex('study_history').where({ id }).first();
    res.json(entry);
  } catch (error) {
    console.error('Error creating study history entry:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

app.put('/api/study-history/:id', async (req, res) => {
  try {
    const { id } = req.params;
    const updatedEntry = req.body;
    delete updatedEntry.id;

    const numUpdated = await knex('study_history').where({ id }).update(updatedEntry);
    if (numUpdated === 0) return res.status(404).json({ error: 'Entry not found' });

    const entry = await knex('study_history').where({ id }).first();
    res.json(entry);
  } catch (error) {
    console.error('Error updating study history entry:', error);
    res.status(500).json({ error: 'Internal server error' });
  }
});

app.delete('/api/study-history/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await knex('study_history').where({ id }).delete();
